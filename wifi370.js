module.exports = function(RED) {
	...
	this.on('input', function(msg) {
		...
		if (node.custom != null) {
		    var input1 = node.color.match(/.{2}/g);
		    var index1;
		    cmds.push("0x56");
		    for (index1 = 0; index1 < input1.length; ++index1) {
			var value1 = "0x" + input1[index1];
			cmds.push(value1);
		    }
		    cmds.push("0xaa");
		}
		...
		if (node.custom != null) {
		    var input2 = node.custom.split(" ");
		    var index2;
		    for (index2 = 0; index2 < input2.length; ++index2) {
			var value2 = "0x" + input2[index2];
			cmds.push(value2);
		    }
		}
	    } else if (node.action == "through") {
		if (msg.payload != null) {
		    var input3 = msg.payload.split(" ");
		    var index3;
